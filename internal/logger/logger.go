...
func isHandledKey(key string) bool {
	return key == ProcessKey || key == AreaKey || key == slog.LevelKey || key == slog.MessageKey || key == slog.TimeKey
}
...
func (h *Handler) WithAttrs(attrs []slog.Attr) slog.Handler {
	return &Handler{handler: h.handler.WithAttrs(attrs), buf: h.buf, replaceAttr: h.replaceAttr, mutex: h.mutex, writer: h.writer, colourise: h.colourise}
}

func (h *Handler) WithGroup(name string) slog.Handler {
	return &Handler{handler: h.handler.WithGroup(name), buf: h.buf, replaceAttr: h.replaceAttr, mutex: h.mutex, writer: h.writer, colourise: h.colourise}
}
...
func PrettyLine(data map[string]any, colourise func(code int, value string) string) (string, error) {
	...
	assert.Assert(ok, "area must be defined when calling PrettyLine")
	...
	header.WriteString(colourise(getProcessColour(process.(string)), process.(string)))
	header.WriteString(":")
	header.WriteString(colourise(getAreaColour(area.(string)), area.(string)))
	header.WriteString(" ")
